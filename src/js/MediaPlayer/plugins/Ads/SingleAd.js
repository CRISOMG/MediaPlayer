...
class SingleAds {
  constructor(ads) {
    if (typeof SingleAds.instance === "object") {
      return SingleAsd.instance;
    }
    this.AdsList = ads;
    this.initAds();
    SingleAds.instance = this;
    return this;
  }
  getInstance() {
    return SingleAds.instance;
  }
  initAds() {
    this.CopyAdsList = [...this.AdsList];
  }
  getAd() {
    if (this.CopyAdsList.length === 0) {
      this.initAds();
    }
    return this.CopyAdsList.pop();
  }
}

const AdsSingleInstance = new SingleAds(ALL_ADS);
const a = AdsSingleInstance;
const b = AdsSingleInstance;
console.log("¿AdsSingleInstance es una unica instancia?", a === b);
export default AdsSingleInstance;
